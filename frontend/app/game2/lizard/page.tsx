...
export default function LizardPage() {
  ...
  const startDrag = useCallback((animal: AnimalCard, cX: number, cY: number, bx: number, by: number) => {
    ...
    setDragId(animal.id); setDragPos({ x: bx, y: by }); setEyeTarget({ x: bx, y: by })
    const mouth = getMouth()
    ...
    const onUp = (e: PointerEvent) => {
      window.removeEventListener("pointermove", onMove)
      window.removeEventListener("pointerup", onUp)
      // Write the drop position back into physics so the animal continues from there
      ...
      setDragId(null); setDragPos(null); setEyeTarget(null)
    }
  ...
  const capture = useCallback(async (animal: AnimalCard, bx: number, by: number) => {
    ...
    setDragPos(null); setDragId(null); setEyeTarget({ x: bx, y: by })
    ...
    if (animal.is_prey) {
      ...
      setTongueEnd(null); setSwallowPos(null); setScore(s => s + 1); setEyeTarget(null)
      ...
      setBubbles([...bRef.current])
      setEyeTarget(null); await sleep(1400)
    }
  ...
  return (
    <div className="wc-cursor" style={{ width: "100vw", height: "100vh", position: "relative", overflow: "hidden", userSelect: "none" }}>
      ...
      <div ref={lzRef} style={{ position: "absolute", top: 0, right: 24, zIndex: 30 }}>
        ...
        >
          <LizardFace state={lzState} size={LIZARD_SIZE} eyeTargetX={eyeTarget?.x} eyeTargetY={eyeTarget?.y} lizardRect={lizardRect} />
        </motion.div>
      </div>
      ...
      {bubbles.map(b => {
        ...
        return (
          ...
          >
            {/* Bare SVG image — no card container */}
            <img
              src={animal.svgSrc}
              alt={animal.label}
              style={{
      ...
      <AnimatePresence>
        {fb && (
          <motion.div style={{ position: "absolute", top: "28%", left: "38%", transform: "translateX(-50%)", zIndex: 50, textAlign: "center", pointerEvents: "none" }}
